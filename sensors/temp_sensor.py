...
def main():
    ...
    try:
        while True:
            # Só tenta publicar leitura se o cliente estiver de fato
            # conectado no momento; caso contrário, registra a leitura como
            # perdida e aguarda a reconexão automática cuidar do resto.
            if not client.is_connected():
                print(f"[{SENSOR_ID}] Sem conexão no momento — leitura não enviada (aguardando reconexão automática)")
                escritor_csv.writerow([
                    time.strftime("%Y-%m-%d %H:%M:%S"), SENSOR_ID, "temperatura", "",
                    "C", QOS, "", "sem_conexao", False
                ])
                arquivo_csv.flush()
